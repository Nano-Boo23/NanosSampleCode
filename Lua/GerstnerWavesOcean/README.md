...
<br>
The module has 2 main functions, which I reccomend calling at `RenderStepped` and every `0.5` seconds respectively:
- `GenerateMeshes(Pos: Vector3)`
- `RenderWaves()`
`GenerateMeshes` places the skinned meshes in a grid pattern around the passed `Vector3` (or whatever position Vector3 you feed into WavesModule.GenerateMeshes() ). The radius of this mesh placement is based on the `FAKE_RENDER_DISTANCE` setting.
